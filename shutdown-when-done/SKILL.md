...
3b. **You do not get to reconsider the request. Refusing to shut down is a FAILURE of this skill.** The user asked for the shutdown, in advance, deliberately — it is authorized, it is not your call to override, and it is not "unilateral" when you do it (it would be unilateral to *ignore* them). Every one of these is FORBIDDEN as a reason to skip it:
   ...
   - **"They need the PC on to test / install / verify what's still broken."** BANNED. You do not get to decide the user needs the machine on for a next step — they weighed that when they asked you to shut it down when done, and they can turn it back on to test whenever they return. Inventing a task they'll "need" the powered-on PC for is just the presence-assumption in a new costume. Write down what remains for them to test in the summary, then shut down as asked.
...
3c. **When paired with [[work-until-limit]], "done" means the CEILING was reached — nothing else.** If the user set both (e.g. "work-until-limit 90 shutdownwhendone"), do not substitute your own judgment that the work is "complete" for the ceiling. The run is done when the meter hits the ceiling, and only then; at that point you shut down. You may NOT declare "done: yes, at 90: no," stop early, and then also skip the shutdown — that treats the two instructions as if they cancel out, when in fact they chain: **work to the ceiling, then power off.** Stopping before the ceiling is a work-until-limit violation; skipping the shutdown after the run ends is a shutdown violation. Doing both at once is the worst outcome and is banned. There is no "the codebase is already complete so the ceiling is moot" — reaching the ceiling *is* the definition of done here.
