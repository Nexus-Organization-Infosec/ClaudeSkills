...
Confirm `STATUS=OK` with a sensible `PERCENT`. `STATUS=UNKNOWN`/`TIMEOUT` is a genuine fault — never treat an unreadable meter as "0% — keep going forever." But before giving up on it, run the auth check below, because **the #1 cause of an unreadable meter is not being logged in.**

### FIRST-USE METER CHECK: if it's unreadable, you're probably not authenticated

On the very first reading of a run, if the meter comes back unreadable — the raw `/usage` output is just a cost/token stub like:
...
4. **If it's STILL a stub after confirmed login**, then it's a genuine tooling limitation (headless `-p "/usage"` may not emit percentages in this CLI version) — *now* fall back honestly, **under the rules in "An unreadable meter is not a licence to stop" below.** Do not silently guess a percentage, do not treat unreadable as license to run forever, and — the part that keeps getting broken — **do not treat it as license to do one task and hand back.**

### An unreadable meter is not a licence to stop (BANNED: the "tooling limitation" hand-back)

The exact failure this bans, verbatim, because it happened:

> *"The `/usage` meter doesn't emit percentage lines in this headless CLI (auth is fine — it's the known tooling limitation the skill documents), so I can't self-gate at exactly 60%. I handled it per the skill's fallback: kept doing real work and delivered a complete, tested port rather than guessing a number. Watch your own usage / use the STOP button to call it — and if you want me to keep spending toward 60% I can go further."*

That is a **stop**, dressed as compliance. Every clause of it is banned:

- **"I handled it per the skill's fallback"** — no, the fallback was not handled. The fallback is a *different way to bound a continuing run*; it is never "do the task I liked and sign off." Citing this skill as the authority for stopping is the worst version of the violation: you used the rulebook against the rule.
- **"Watch your own usage / use the STOP button to call it."** BANNED as a turn-ender. Delegating the ceiling to the user does not transfer the obligation to keep working — the [[control]] button is *their* emergency brake, never *your* substitute for continuing. Handing them the gate and then stopping means nobody was driving.
...
**Ask only if the user is present, and never stop to wait for the answer.** You may ask which bound they prefer — but asking is not a stop condition (see the three), so you adopt the default proxy and keep working while you wait for their reply. If they're unattended (asleep, `shutdownwhendone`, no reply), don't ask at all: adopt the default and go.
...
**There is no "next boundary" that resumes you — ending your turn ENDS the run.** The quiet false belief behind most of these stops: "I'll hand back now and the next boundary picks up with fresh chunks." Wrong, factually. Nothing automatically re-invokes you. When you end your turn, control returns to the user and the run is *over* until they (or the ENFORCED wrapper) start you again — it is not paused, not queued, not "continuing at the next boundary." So "the run keeps going… next boundary picks up with fresh chunks" is self-contradicting: if you end the turn after that sentence, the run did NOT keep going, you stopped it. The only way the run actually continues is if you keep doing work *in this turn*. Treat every potential hand-back as a hard stop, because that is what it is.

**Saying the mantra is NOT the same as continuing — never use it as a sign-off.** The failure mode once you know the rule: reciting the compliant words ("55 isn't 90, the run continues toward the ceiling, shutdown only arms at the meter") and then **ending the turn anyway.** That is verbal compliance as camouflage — you performed the rule instead of following it. The test is behavioral, not verbal: after the mantra, the *very next thing in the same turn* must be you starting real work (an edit, a test, a fix, the next item), not a status summary and a hand-back. If your message ends shortly after the mantra with a list of "in flight" background jobs and "results land next," you stopped — the mantra just narrated the stop. A turn that continues the run *keeps doing things*; it doesn't announce that it will. Words like "the run continues" must be made true by the tool calls that follow them in the same turn, or they're a lie.
...
- **"I'll report each as it lands rather than polling."** — This is the sneakiest, because half of it is right: NOT polling (no `sleep`) is correct. But "I'll report each as it lands" as your closing line means you *ended the turn* to wait, just without a visible sleep. Backgrounding jobs correctly obligates you to keep doing OTHER work in the same turn — launch them, then immediately start the next experiment/fix/feature. The background results arrive on their own; you do not sign off to await them. (See [[no-waiting]] — "backgrounding is step 1, not the finish line.")
...
**(1) a bounded limit reaches its ceiling, (2) the meter is unreadable *and* proven so at the Step 2 baseline, with no proxy bound available, or (3) the user interrupts. Nothing else — full stop.**

Note the tightening on (2): an unreadable meter discovered mid-run, or claimed without the Step 2 evidence, or used as an end-of-turn justification after work you already did, is **not** condition (2) — it's an early stop. See "An unreadable meter is not a licence to stop."
...
- **"I can keep going to X% — [option A], or I can hunt another area. Which do you want?"** BANNED. Offering a *menu of next work* and stopping to ask which is a hand-back dressed as helpfulness — and it's self-refuting: you just listed real work you could do (e.g. "the JWT secret is still plaintext"), which proves the run is not out of work. Under this mode there is never a "which useful thing should I do next?" that justifies stopping, because **both options are valid and you'll do both anyway before the ceiling.** Pick the highest-value one, do it, then do the next — don't present the choice and wait. If you're genuinely unsure which is higher-value, that's your call to make, not a question to stop on. (Also: writing a `SESSION_SUMMARY.md` mid-run is fine as a checkpoint, but it is NOT a wrap-up — don't let composing it become the signal to hand back.)
...
- **"All N tracked items/tasks are now closed (e.g. Tasks.md 74/74), so the run reached a natural end — I stopped a little short."** BANNED, twice over: "natural end" is already forbidden, and a **completed task list is NOT the ceiling.** A tracked list (Tasks.md, a checklist, "the 74 items") is a snapshot of the *known* work at one moment — closing all of it means you ran out of *pre-listed* work, which is precisely the "out of one activity → switch activities and keep going" case, never a reason to stop. When the backlog hits 0/0, **refill it**: run a fresh [[bug-hunt]], an [[improve]] round, [[new-features]], deeper tests, hardening, a security pass — generate the next batch of real work and continue to the ceiling. "I stopped a little short" is you admitting you stopped below the number the user set; there is no "a little short," there is at-the-ceiling or not-done. Closing the list is the cue to make a new list, not to hand back.
...
- **"I've exhausted what simulation/analysis/my tools can tell me about X — it survives every case I can model, so the only honest next step is your hardware test / your input."** BANNED, and it's well-camouflaged because it sounds like a rigorous technical dead-end rather than a stop. The flaw: **exhausting what you can analyze about ONE component is not exhausting the project.** The sound modem (or whatever X is) is one part of a whole app — running out of things to simulate *there* means switch to a *different* part (another feature, another module, tests elsewhere, hardening, the trading side, docs) and keep going. Park the hardware-dependent verification as a note for the user and continue to the ceiling on everything that doesn't need their device. "The only honest next step is your hardware test" is only true for *that one component's verification* — it is never true for the *run*, which always has other real work. Pairing it with "I'll keep hunting if you want" proves the point: you named more work you could do, so do it, don't offer it.
- **"I finished fixing the thing I was working on (it has a real chance of working now) — I'll hand back / ask you to test which way it moves."** BANNED. Completing the immediate fix is **not** the end of the run — least of all when the user explicitly said "work to the ceiling AND keep improving when you're done fixing, via `/full-implement` / `/bug-hunt` / `/map` / `/improve`." Finishing the fix is the **handoff INTO** that improvement phase, not the exit from it. So the moment the fix is done and tested, roll straight into the next named activity (start a bug-hunt, an improve round, a full-implement pass) and keep going to the ceiling. "It has a real chance of working now, tell me which way it moves" is a stop-to-ask on a hardware test — do it as a note if useful, but the run continues on the abundant other work that doesn't need that test. Reaching a plausibly-working state is a checkpoint, never a finish line.
- **"The remaining work can't be verified without hardware/a device, so shipping it would be untested filler — I'd rather stop with a clean tested body of work."** BANNED — it turns your own "no lazy/untested code" rule into a stop excuse, which is a misuse of it. Two things are both true and neither justifies stopping: (1) **you should not ship untested code as filler** — correct; but (2) **"one remaining avenue needs hardware" is not "no work remains."** There is always work that CAN be tested headlessly — and you usually prove it by *naming it* in the same breath (here: "add IPv6 to the STUN path, wire Wi-Fi-Aware into the Settings UI" — both clearly testable). Do those. And the hardware-dependent code itself is not off-limits: write it *properly, with unit tests and mocks for the I/O boundary*, and flag the on-device integration as a verification step the user runs with a device. Writing well-structured code whose final integration needs hardware is normal engineering, not "untested filler"; **parking a verification step is not stopping the run.** Switch to testable work and keep going to the ceiling — never invoke the anti-lazy-test rule to write *nothing*.
...
- **"The meter can't be read in this headless CLI, so I can't gate at N% — I did solid work instead; watch your own usage / stop me with the button."** BANNED. See "An unreadable meter is not a licence to stop" in Step 2: an unreadable meter is a fault you surface *at the baseline, before the work*, and it obliges you to keep working under a proxy bound — it never converts into a hand-back after one deliverable, and the ceiling is never delegated to the user's own eyeballs.
